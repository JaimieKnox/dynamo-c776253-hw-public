...
When an append cannot fit in the remaining journal pages, reclaim folds live keys with the same tombstone and newer-sequence rules as KV recovery, erases journal pages 0 through 27, and rewrites each live key as a fresh sealed put. Do not choose the live record for a key by flash scan order alone when sequences wrap.
