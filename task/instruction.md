...
Job packs sit under `/app/jobs`. The `sample_*` packs stay green on the shipped tree. The `h_*` packs exercise meta policy bits, boot ranking, antipode generation ties, torn meta, and tombstone reclaim, sometimes in combination.
