...
- `force_meta_epoch` with `floor` and `epoch` (plants both meta copies at a chosen epoch and floor while preserving the current policy word, never clearing policy bits)
